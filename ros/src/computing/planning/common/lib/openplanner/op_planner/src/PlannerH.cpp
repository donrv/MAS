...
PlannerH::~PlannerH()
{
 ...
 double PlannerH::PlanUsingAStar(const WayPoint& start, const WayPoint& goal, GridWorld& map, std::vector<WayPoint>& genSmoothedPath, CAR_BASIC_INFO carInfo, const double pathDensity, const double smoothFactor)
 {
	 HybridPlanner hyb_planner(&map, carInfo);
	 vector<vector<WayPoint> > genPaths;
	 hyb_planner.HybridAStarPlan(start, goal, genPaths, 0);
	 if(genPaths.size()>0)
	 {
		 genSmoothedPath = genPaths.at(0);
		 return genSmoothedPath.size();
	 }
...
double PlannerH::PlanUsingReedShepp(const WayPoint& start, const WayPoint& goal, vector<WayPoint>& generatedPath, const double pathDensity, const double smoothFactor)
 ...
 double PlannerH::PlanUsingReedSheppWithObstacleDetection(const WayPoint& start, const WayPoint& goal,GridWorld& map, vector<WayPoint>& genSmoothedPath,
...
// 		else
 		pCellRet = map.GetCellFromPoint(GPSPoint(p.pos.x, p.pos.y,0,0));
